...
If you're using Claude Code, running `/release` in this repo drives all of the steps below in one go (version bump, `README.md`/`AGENTS.md`/`CHANGELOG.md` updates, typecheck/test, commit, push, the `gh release create --prerelease` call, and then waiting for the resulting `publish-npm.yml` CI run to finish and reporting whether it actually succeeded) — see `.claude/commands/release.md`. The manual steps:
